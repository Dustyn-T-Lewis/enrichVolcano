...
1.  `read_study()` reads your DA results, and optionally the matrix,
    sample sheet and contrasts, from one workbook; `as_da()` reads a
    single DA table from limma, limpa, proteoDA, MSstats, proDA,
...
3.  `dedup()` (optional) flags redundant terms so a figure shows each
    signal once. p-values are never changed.
4.  A plot function draws from the object: `volcano_ring()` for one
...
and 1.1.0 added the study readers and `run_enrichment()`; see `NEWS.md`.
Earlier releases stay installable from their tags.
