...
- tick lines from each pathway arc to its leading-edge genes inside the
  volcano
- a multi-contrast grid composer (`volcano_ring_grid()`) for
  side-by-side panels
...
`vignette("input-contract")` for the column conventions plus a limma /
DESeq2 / edgeR / DEP / fgsea / clusterProfiler / enrichR cross-walk.
