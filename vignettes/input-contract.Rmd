...
`DEP::get_results()` returns a wide tibble with one row per protein and one
group of columns per contrast (`<contrast>_ratio`, `<contrast>_p.val`,
`<contrast>_p.adj`). `volcano_ring()` wants one row per protein **per
contrast**, so a single `tidyr::pivot_longer()` reshapes it:
...
Enrichment from `fgsea` is already in the shape `volcano_ring()` expects —
no reshape, no rename:
