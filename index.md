...
enrichment score](reference/figures/README-hero-1.png)

enrichVolcano starts from differential-abundance results you already
have and ends in enrichment figures:
...
volcano_ring(study$da, res$fgsea, contrast = "Aging")
```

1.  [`read_study()`](https://Dustyn-T-Lewis.github.io/enrichVolcano/reference/read_study.md)
    reads your DA results, and optionally the matrix, sample sheet and
    contrasts, from one workbook;
    [`as_da()`](https://Dustyn-T-Lewis.github.io/enrichVolcano/reference/as_da.md)
    reads a single DA table from limma, limpa, proteoDA, MSstats, proDA,
    msqrob2, prolfQua or ProtRank.
2.  [`run_enrichment()`](https://Dustyn-T-Lewis.github.io/enrichVolcano/reference/run_enrichment.md)
...
3.  [`dedup()`](https://Dustyn-T-Lewis.github.io/enrichVolcano/reference/dedup.md)
    (optional) flags redundant terms so a figure shows each signal once.
    p-values are never changed.
4.  A plot function draws from the object:
    ...
    to compare two contrasts term by term (concordance or reversal).
...
walks every step on the bundled example, including how to get each
gene-set collection.

## Cite the methods you used

Cite the methods behind your results, whether enrichVolcano ran them or
you did:

- limma: Ritchie ME et al. 2015, *Nucleic Acids Res* 43(7),
...
- EnrichmentMap
  ...
  Merico D et al. 2010, *PLoS ONE* 5(11):e13984
- Hallmark gene sets: Liberzon A et al. 2015, *Cell Systems* 1(6),
...
and 1.1.0 added the study readers and
