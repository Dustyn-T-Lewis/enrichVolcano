...
A pathway earns its place because a handful of proteins drive it. When
the enrichment table carries those leading-edge genes,
[`volcano_ring()`](https://Dustyn-T-Lewis.github.io/enrichVolcano/reference/volcano_ring.md)
draws a thin tick from each arc to its genes inside the volcano, so a
pathway reads back to the points that produced it. The YvO enrichment
table above has no gene column, so the arcs above draw without ticks.
...
![Volcano in ring with leading-edge tick
lines](getting-started_files/figure-html/ticks-1.png)

`genes_col` is auto-detected from `leading_edge`, `leadingEdge`,
`core_enrichment`, or `Genes`;
[`vignette("input-contract")`](https://Dustyn-T-Lewis.github.io/enrichVolcano/articles/input-contract.md)
lists the conventions each upstream tool uses.
...
collecting one shared NES legend. Here are the trained young and old
contrasts, each curated to its top pathways:
...
panels](getting-started_files/figure-html/grid-1.png)
