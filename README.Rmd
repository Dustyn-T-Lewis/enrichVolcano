...
```{r hero, echo = FALSE, fig.alt = "A volcano plot ringed by NES-coloured enrichment arcs", fig.width = 7, fig.height = 7, dpi = 110}
library(enrichVolcano)
da <- read.csv(system.file("extdata", "examples", "yvo_da.csv", package = "enrichVolcano"))
en <- read.csv(system.file("extdata", "examples", "yvo_enrichment.csv", package = "enrichVolcano"))
ctr <- "Training_Young"
da1 <- da[da$contrast == ctr, , drop = FALSE]
names(da1)[names(da1) == "adj.P.Val"] <- "padj"

# You choose which pathways ring the volcano; here, top 7 per direction.
sig <- en[en$contrast == ctr & en$padj < 0.05 & is.finite(en$NES), , drop = FALSE]
sig <- sig[order(sig$padj), ]
top <- rbind(head(sig[sig$NES > 0, ], 7), head(sig[sig$NES < 0, ], 7))

suppressWarnings(volcano_ring(da1, top, title = "enrichVolcano", subtitle = "one contrast, one figure"))
```
...
- **Ring:** one arc per enriched pathway. Arc fill is the NES
  (red up, blue down by default); arc thickness is a magnitude you
  pick (`-log10(padj)` or gene-set size).
...
`legend_position`, `legend_width`) has a sensible default, so the call above
is the whole figure.
