...
test_that("the default combined rule at 0.375 flags B and C under A", {
  x <- dedup(make_toy_dedup_enrichment(), make_toy_gene_sets())
  f <- flags(x)
  expect_identical(
...
test_that("Jaccard at 0.5 keeps C, whose Jaccard with A is only 0.4", {
  x <- dedup(make_toy_dedup_enrichment(), make_toy_gene_sets(), similarity = "jaccard")
  f <- flags(x)
...
test_that("an explicit cutoff overrides the rule's default", {
  x <- dedup(make_toy_dedup_enrichment(), make_toy_gene_sets(), similarity = "jaccard", cutoff = 0.9)
...
test_that("each contrast is walked in its own padj order", {
  ...
  r <- dedup(x, make_toy_gene_sets())@results
  in_l <- r[r$contrast == "L", ]
...
test_that("terms without a gene set are kept and counted", {
  ...
  sets$SET_B <- NULL
  expect_message(x <- dedup(make_toy_dedup_enrichment(), sets), "1 term")
  expect_identical(unname(flags(x)["SET_B"]), "kept")
})
...
test_that("earlier dedup columns are replaced, not duplicated", {
  ...
  x@results <- r
  y <- suppressMessages(dedup(x, make_toy_gene_sets()))
...
test_that("bad inputs are refused with classed errors", {
  x <- make_toy_dedup_enrichment()
  sets <- make_toy_gene_sets()
  expect_error(dedup(x@results, sets), class = "enrichVolcano_input_error")
  expect_error(dedup(x, unname(sets)), class = "enrichVolcano_input_error")
  expect_error(dedup(x, "SET_A"), class = "enrichVolcano_input_error")
  expect_error(dedup(x, sets, cutoff = 1.5), class = "enrichVolcano_param_error")
